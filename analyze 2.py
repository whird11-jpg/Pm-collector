...
# CHANGED 2026-08-10, BEFORE looking at any test result. Rationale: 0.015 on a
# 50c contract is a 3% edge, thin enough that execution slippage likely eats it.
# Required sample scales as 1/effect^2, so 0.03 cuts 8,721 observations to ~2,180.
# The cost: a real 0.015 edge would be missed. Recorded here so the change is
# visible rather than quietly fitted after the fact.
TARGET_EFFECT = 0.030
...
def status(snaps, outcomes):
    ...
    if "asset" in snaps.columns:
        by_a = snaps.dropna(subset=["asset"]).groupby("asset").slug.nunique()
        if len(by_a):
            print(f"  markets by asset   : "
                  + ", ".join(f"{k}={v}" for k, v in by_a.items()))
        missing = snaps.asset.isna().sum()
        if missing:
            print(f"  rows predating multi-asset: {missing:,} (BTC only)")
