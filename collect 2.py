"""
collect.py — snapshot open crypto up/down markets, including the real order book.

Run on a schedule. Each run appends one row per open market to
...
these rows measure the spread you would actually have paid.
    ...
    python collect.py --duration 300 --every 60
"""
...
# Six assets run the same up/down markets on the same clock. Pooling them is
# legitimate for this research question: favourite-longshot bias is a claim about
# how people price probabilities, not a claim about Bitcoin. ~6x the sample rate.
...
FIELDS = [
    "captured_at", "slug", "asset", "interval_min", "market_id", "yes_token",
...
def current_windows(now_ts):
    """Slugs for the in-progress window of each interval and asset, plus next."""
    out = []
    for asset in ASSETS:
        for label, mins in INTERVALS.items():
            ...
            for start in (base, base + step):
                out.append((f"{asset}-updown-{label}-{start}",
                            asset, label, mins, start))
    return out
...
def fetch_book(token_id):
    """Top of book plus depth. The data the historical API never gave us."""
...
def sweep():
    """One pass over every open market across every asset."""
    ...
    def capture(spec):
        slug, asset, label, mins, start = spec
        m = fetch_market(slug)
        ...
        return {
            "captured_at": now.isoformat(), "slug": slug, "asset": asset,
            "interval_min": mins, "market_id": m["market_id"],
    ...
    specs = current_windows(now_ts)
    rows = []
    # 36 lookups per sweep now; serial would not finish inside a minute
    with cf.ThreadPoolExecutor(max_workers=12) as pool:
        for r in pool.map(capture, specs):
            if r:
                rows.append(r)
    ...
    print(f"{now:%H:%M:%S}  wrote {len(rows)} rows  "
          + " ".join(f"{a}={n}" for a, n in sorted(by_asset.items())))
    return len(rows)
...
def main():
    ...
    ap.add_argument("--duration", type=float, default=300,
                    help="minutes to keep sweeping (job limit is 6h)")
